...
pub fn parse(input: &str, filename: &str) -> Result<ParsedDocument, JianPuError> {
    ...
    for section in sections {
        match section.kind {
            SectionKind::Metadata => raw_metadata = Some((section.content, section.content_offset)),
            SectionKind::Score => raw_score = Some((section.content, section.content_offset)),
            SectionKind::Lyrics => raw_lyrics = Some((section.content, section.content_offset)),
        }
    }
    ...
    let (meta_content, meta_offset) = raw_metadata.ok_or_else(|| {
        JianPuError::new(zero_span.clone(), "missing [metadata] section")
    })?;
    let (score_content, score_offset) = raw_score.ok_or_else(|| {
        JianPuError::new(zero_span.clone(), "missing [score] section")
    })?;
    let (lyrics_content, _) = raw_lyrics.ok_or_else(|| {
        JianPuError::new(zero_span, "missing [lyrics] section")
    })?;
