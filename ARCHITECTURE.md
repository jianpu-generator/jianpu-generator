...
- When rows merge, the surviving row's label is the space-joined concatenation of each source row's original label (its part abbreviation).
...
| **Group** | A named alias for a specific set of part abbreviations, declared in the optional `# groups` section (`<display-name> [<abbreviation>] = <members>`), parsed into `GroupSection`/`GroupDef` by `group_parser::parse_group()`. Has no effect on rendering, MIDI output, or row labels beyond the legend: `part_info::list_groups_from_source()` re-parses the source into `GroupInfo { abbreviation, display_name, members }` entries, `lib::filter_group_list()` drops a group when a track filter (`enabled_tracks`) excludes all of its `members`, and `build_header()` appends each surviving group (whose `abbreviation != display_name`) to `Header.part_list` alongside `PartInfo` entries, so it renders in the same part-list legend rows. |
