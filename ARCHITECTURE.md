...
- Key types: `Score`, `MultiPartMeasure` (carries `bpm`/`key`/`time_signature`/`label` plus the navigation markers `dc_al_coda`/`to_coda`/`coda` (D.C. al Coda) and `segno`/`ds_al_coda` (D.S. al Coda), all non-persisting — see **Navigation expansion** in the glossary), `PartRow` (Timed), `PartSlice` (carries `soundfont`, `volume`, `octave_offset`), `Notes`, `NoteEvent` (includes `Percussion(GroupedPercussionHit)`), `GroupedNote`, `GroupedRest`, `GroupedChordNote` (`GroupedNote`/`GroupedChordNote` use `tie_to_next_span` + `tie_to_next()` accessor), `GroupedPercussionHit` (mirrors `GroupedNote` minus pitch/accidental/octave, plus `event_span`; `slur_key()` returns the inert `SlurKey::Rest` sentinel since hits have no pitch to differentiate slur arcs by), `GroupedMeasure` (intermediate: notes + paired lyrics per measure)
...
| **Decoration** | Measure-level metadata attached to a `MeasureBlock`: BPM, time signature, section label, bar number, D.C. al Coda navigation markers (`dc_al_coda`/`to_coda`/`coda`), D.S. al Coda navigation markers (`segno`/`ds_al_coda`, reusing `to_coda`/`coda`). |
...
| **Navigation expansion** | `midi::expand_navigation(score: &Score) -> Result<Score, IrrecoverableError>`, a `Score -> Score` pass applied on the whole-score MIDI/WAV export path (after `apply_track_filter`, before `midi::write_midi`) and on `measure_start_times_from_source`. Rebuilds `measures` by index to reflect either the `dcalcoda`/`tocoda`/`coda` markers (play from the start through the `dcalcoda` measure, restart from the start through the `tocoda` measure, then jump to the `coda` measure through the literal end) or the `segno`/`dsalcoda`/`tocoda`/`coda` markers (same, but pass 2 restarts from the `segno` measure instead of the start). Exactly one scheme may be used per score; mixing them is an error. Identity when no markers are present. Thin wrapper over `midi::expand_navigation_with_origins(score: &Score) -> Result<(Score, Vec<usize>), IrrecoverableError>`, which additionally returns a same-length `Vec<usize>` mapping each expanded measure back to its index in the written `Score`. `midi::earliest_playback_position(origins: &[usize], written_index: usize, min_pos: usize) -> Option<usize>` finds the smallest expanded position `>= min_pos` whose origin equals `written_index`; `midi::expand_for_measure`/`midi::expand_for_measure_range` use it (plus a last-occurrence scan for the range's end) to translate written measure-index/range requests into playback-order positions, and are called from `lib.rs` for the four measure-index entry points (`write_wav_for_measure_from_source`, `write_wav_for_measure_range_from_source`, `measure_start_times_from_source`, `measure_start_times_for_range_from_source`), falling back to the written index/range when a measure has no reachable playback position (e.g. it lies strictly between the `dcalcoda`/`dsalcoda` measure and the `coda` measure). SVG/PDF export never calls any of this — those always render measures in written order. |
