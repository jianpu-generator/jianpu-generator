 ...
 * resolving each note's `sourcePartIndex`/`note_id`, so when a caller
 * passes it, the returned `sourcePartIndex` already lands in the same
 ...
 * computed *without* a `visible_tracks` filter (`sourcePartIndex` still
...
export function remapNoteTimingsToVisiblePartIndex(
  ...
  return timings.flatMap((timing) => {
    const compactedIndex = compactedIndexByWrittenIndex.get(
      timing.sourcePartIndex,
    )
    return compactedIndex === undefined
      ? []
      : [{ ...timing, sourcePartIndex: compactedIndex }]
  })
}
