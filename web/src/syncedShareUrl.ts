...
// The KV worker only needs an opaque share name, not a UUID shape — 64 bits
// keeps accidental collisions negligible (shares are never deleted, see
// `live-share-worker/src/index.ts`) while staying short, since every char
// here lands directly in a copy-pasted URL.
...
const DEVICE_SECRET_KEY = 'jianpu:synced-share-device-secret:v1'
...
export interface SyncedSharePayload {
   ...
   * authoritative — the share's stored KV doc is the source of truth, so a
   * rename after sharing doesn't invalidate the link, it just makes this
   * cosmetic copy stale. */
  filename?: string
}

export interface SyncedShareIdentity {
  shareId: string
  ownerToken: string
}
 ...
 * file's stable `fileId` to derive that file's share id and owner token, so
 * the same file always reproduces the same share link without persisting a
 * shareId/ownerToken pair per file. Kept separate from
 * `deriveSyncedShareIdentity` so that function stays a pure,
 * unit-testable derivation. */
...
/** Derives a stable `{shareId, ownerToken}` pair for a file from a device
 * secret (see `getOrCreateDeviceSecret`) and the file's `fileId`.
 * Deterministic: the same file + secret always reproduces the same share
 * link, so stopping and re-starting a sync doesn't hand out a new one. The
 * ownerToken can't be derived from the (publicly shared) shareId alone —
 * both are independent HMAC outputs keyed by the same secret, and the
 * secret itself is never sent anywhere. */
export async function deriveSyncedShareIdentity(
  secret: string,
  fileId: string,
): Promise<SyncedShareIdentity> {
  const [shareBytes, ownerBytes] = await Promise.all([
    hmacSha256(secret, `share:${fileId}`),
    hmacSha256(secret, `owner:${fileId}`),
  ])
  return {
    shareId: toBase64Url(shareBytes.slice(0, 8)),
