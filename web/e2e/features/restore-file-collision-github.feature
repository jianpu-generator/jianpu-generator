Feature: Restoring a bin file that collides with an active file via GitHub storage backend
  ...
  Scenario: Restoring a file that collides with an active file renames it via the GitHub storage backend
    ...
    Then exactly one "original.jianpu" tab exists and no "original 2.jianpu" tab exists
    And the file actions bin trigger shows "Bin (1)" before the collision restore
    When I open the bin
    Then the bin lists the colliding restorable file "original.jianpu"
    ...
    Then a "original 2.jianpu" tab appears within 5 seconds
    And both "original.jianpu" and "original 2.jianpu" tabs exist exactly once each
    And the active tab is now the renamed restored file "original 2.jianpu"
    ...
    Then both "original.jianpu" and "original 2.jianpu" tabs exist exactly once each after reload
