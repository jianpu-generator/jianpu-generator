Feature: Playback cursor selection with hidden part
  ...
  Scenario: Playing a drag-selected part plays its notes and moves the playback cursor onto them, even when an earlier part is hidden
    ...
    When I Cmd/Ctrl-drag to select Bass's two notes at the compacted part-index
    Then 2 notes are drag-selected at part-index 1
    And 2 notes are drag-selected in total, as seen in playback cursor selection with hidden part
