...
When(
  "I click near the top of note {int}'s click target without dragging",
...
Then(
  'only syllable {int} of verse {int} is drag-selected',
  async ({ page }, noteId: number, verse: number) => {
    await expect(
    ...
    await expect(
      page.locator(
        `[data-tag="lyric"][data-lyric-drag-selected][data-note-id="${noteId}"][data-verse="${verse}"]`,
      ),
    ).toHaveCount(1)
  },
)
...
Then(
  'no note is drag-selected by the syllable-level interaction',
  async ({ page }) => {
    await expect(
...
Then('no lyric syllable is drag-selected', async ({ page }) => {
  await expect(
    page.locator('[data-tag="lyric"][data-lyric-drag-selected]'),
  ).toHaveCount(0)
})
...
Then(
  '{int} lyric syllables in total are drag-selected',
...
Then(
  'exactly {int} note is drag-selected via the note click target',
  async ({ page }, count: number) => {
    await expect(
      page.locator('[data-tag="note"][data-note-drag-selected]'),
    ).toHaveCount(count)
...
Then(
  'exactly {int} notes are drag-selected via the note click target',
  async ({ page }, count: number) => {
    await expect(
      page.locator('[data-tag="note"][data-note-drag-selected]'),
    ).toHaveCount(count)
...
Then(
  'syllable {int} of verse {int} is drag-selected but syllable {int} of verse {int} is not',
  async (
    { page },
    noteIdA: number,
    verseA: number,
    ...
    await expect(
      page.locator(
        `[data-tag="lyric"][data-lyric-drag-selected][data-note-id="${noteIdA}"][data-verse="${verseA}"]`,
      ),
    ).toHaveCount(1)
    // Verse 0's corresponding syllable must not also be marked selected.
    await expect(
      page.locator(
        `[data-tag="lyric"][data-lyric-drag-selected][data-note-id="${noteIdB}"][data-verse="${verseB}"]`,
      ),
    ).toHaveCount(0)
  },
)
