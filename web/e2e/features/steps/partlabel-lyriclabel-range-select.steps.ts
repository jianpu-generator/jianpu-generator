...
Then(
  '{int} notes are drag-selected in total, as seen in partlabel lyriclabel range select',
...
Then(
  '{int} syllables are drag-selected in total, as seen in partlabel lyriclabel range select',
  async ({ page }, count: number) => {
    ...
    await expect(
      page.locator(
        '[data-tag="lyric"][data-lyric-drag-selected][data-part-index="1"][data-verse="1"]',
      ),
    ).toHaveCount(0)
  },
)
...
Then(
  'no note in measure 2 is drag-selected, as seen in partlabel lyriclabel range select',
  async ({ page }) => {
