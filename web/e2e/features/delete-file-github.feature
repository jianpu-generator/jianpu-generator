Feature: Delete file via GitHub storage backend
  ...
  Scenario: Deleting a file persists via the GitHub storage backend
    ...
    And I select the "original" tab to delete it
    ...
    And the bin lists the deleted file "original"
    And the active tab falls back to "01-pitches"
    When I reload the page after deleting
    Then the file list no longer shows the deleted file "original" after reload
    And the file actions bin trigger shows "Bin (1)" after reload
    And the bin lists "original" after reload
