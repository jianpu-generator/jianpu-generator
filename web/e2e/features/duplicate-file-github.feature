Feature: Duplicate file via GitHub storage backend
  ...
  Scenario: Duplicating a file persists via the GitHub storage backend
    ...
    And I select the "original" tab to duplicate it
    ...
    And the active tab becomes the duplicate "original 2"
    ...
    Then the file list still shows both "original" and "original 2" tabs
