Feature: Lyric range selection resolves against a lyric label
  ...
  Scenario: Clicking a verse-0 syllable then a verse-1 label in a later system selects both verses' syllables in the swept measure range
    ...
    Then 4 syllables are drag-selected in total, as seen in lyric lyriclabel range select
    And no syllable in measure 2 is drag-selected, as seen in lyric lyriclabel range select
    And no note is drag-selected, as seen in lyric lyriclabel range select
