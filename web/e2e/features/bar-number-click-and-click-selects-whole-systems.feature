Feature: Click-and-click from a bar number selects whole systems
  ...
  # click-and-click range gesture (no drag, no modifier — the same two-click
  ...
  # element is (see the 'bar-number-system' mode in `previewDragState.ts`).
  #
  ...
  Scenario: Clicking a bar number twice selects every part in that bar number's system
    ...
    Then 4 drag-selected notes belong to part index 0, as seen in bar number click-and-click selects whole systems
    And 4 drag-selected notes belong to part index 1, as seen in bar number click-and-click selects whole systems
    And 8 notes are drag-selected in total, as seen in bar number click-and-click selects whole systems
  ...
  Scenario: Clicking a bar number then a note in the next system's first measure still selects that whole system, not just the clicked measure
    ...
    Then 8 drag-selected notes belong to part index 0, as seen in bar number click-and-click selects whole systems
    And 8 drag-selected notes belong to part index 1, as seen in bar number click-and-click selects whole systems
    And 16 notes are drag-selected in total, as seen in bar number click-and-click selects whole systems
    And the play-measure button reads Selection, as seen in bar number click-and-click selects whole systems
