Feature: Synced share button
  ...
  # GitHub sign-in is required to create/own a Synced Share (no more
  # anonymous device-secret ownership) -- every scenario below signs the
  # owner in first (via "the owner is signed in with GitHub as ...", which
  # pre-seeds a Synced Share GitHub connection so "Sync" starts a share
