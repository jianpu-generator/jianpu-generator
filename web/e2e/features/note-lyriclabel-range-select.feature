Feature: Note range selection resolves against a lyric label
  ...
  Scenario: Clicking a note in one part then a different part's verse label selects that part's notes and the label's own verse
    ...
    Then 4 notes are drag-selected in total, as seen in note lyriclabel range select
    And 2 syllables are drag-selected in total, as seen in note lyriclabel range select
    And no note in measure 2 is drag-selected, as seen in note lyriclabel range select
