Feature: Rename file via the local storage backend
  ...
  Scenario: SVG preview persists after renaming the active file
    ...
    And I fill the active tab's rename input with "renamed.jianpu" and press Enter
    Then the active tab shows the name "renamed.jianpu"
    And the file switcher trigger shows the renamed file "renamed.jianpu"
