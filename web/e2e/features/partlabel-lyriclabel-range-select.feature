Feature: Part label range selection resolves against a lyric label
  ...
  Scenario: Clicking one part's label then a different part's verse label selects every note in the swept range and just the label's own verse
    ...
    Then 4 notes are drag-selected in total, as seen in partlabel lyriclabel range select
    And 2 syllables are drag-selected in total, as seen in partlabel lyriclabel range select
    And no note in measure 2 is drag-selected, as seen in partlabel lyriclabel range select
