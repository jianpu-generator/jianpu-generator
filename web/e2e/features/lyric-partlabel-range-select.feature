Feature: Lyric range selection resolves against a part label
  ...
  Scenario: Clicking a syllable in one part then a different part's label selects every note in the swept range and just the syllable's own verse
    ...
    Then 4 notes are drag-selected in total, as seen in lyric partlabel range select
    And 2 syllables are drag-selected in total, as seen in lyric partlabel range select
    And no note in measure 2 is drag-selected, as seen in lyric partlabel range select
