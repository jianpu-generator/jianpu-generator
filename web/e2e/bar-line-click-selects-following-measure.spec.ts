...
test('clicking a few pixels into the hit-padding of a mid-system bar line still selects the following measure', async ({
  page,
}) => {
...
test("clicking a system's last bar line selects the measure before it, not the next system's first measure", async ({
  page,
}) => {
  await loadFixture(page)
  await page.goto('/')
