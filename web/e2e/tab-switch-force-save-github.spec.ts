...
test('switching the active file tab force-flushes a pending debounced GitHub save', async ({
  ...
  const aTab = page.locator('.file-tab-name', { hasText: 'a' })
  await aTab.waitFor({ timeout: 15_000 })
  await aTab.click()
  await expect(fileSwitcherTrigger(page)).toContainText('a')
  ...
  await page.reload()
  await openFileList(page)
  await page.locator('.file-tab-name', { hasText: 'a' }).waitFor({
    timeout: 15_000,
  })
  await page.locator('.file-tab-name', { hasText: 'a' }).click()
